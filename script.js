...
/* =============================================
   BACKGROUND MUSIC
   Tries to autoplay immediately. If the browser
   blocks it, plays on the very first interaction.
   Button is mute/unmute only — never blocks play.
   ============================================= */
(() => {
  ...
  const savedMuted = localStorage.getItem('ib-music-muted') === 'true';

  function syncBtn() {
    const m = audio.muted || audio.paused;
    iconOn.style.display  = m ? 'none' : '';
    iconOff.style.display = m ? ''     : 'none';
    btn.classList.toggle('playing', !m);
  }
  ...
  function startMusic() {
    ...
    audio.play().then(() => {
      ...
      document.removeEventListener('scroll',     startMusic, { passive: true });
      document.removeEventListener('click',      startMusic);
      document.removeEventListener('touchstart', startMusic, { passive: true });
  ...
  // Fallback: fire on the very first user interaction
  ...
  btn.addEventListener('click', (e) => {
    ...
    localStorage.setItem('ib-music-muted', audio.muted);
    syncBtn();
  });
})();LIC SITE SCRIPT
   script.js
